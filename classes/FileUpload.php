...
class FileUpload
{
    ...
    static function arrImageValidate(array $imgArray, $debug = false)
    {
        $uploadOk = 1;
        if ($debug) {
            print_r($imgArray);
        ...
        $target_file = self::$target_dir . basename($imgArray["name"]);
        ...
        // Check file size
        if (intval($imgArray["size"]) / 1024 > 1500) {
            if ($debug) echo "Sorry, your file is too large.";
            $uploadOk = 0;
            return false;
        }
    ...
    static function arrImageUpload(array $imgArray, $debug = false)
    {
        $uniqueFileName = uniqid() . '.' . strtolower(pathinfo($imgArray["name"], PATHINFO_EXTENSION));
        $target_file = self::$target_dir . basename($uniqueFileName);
        // $target_file = self::$target_dir . basename($imgArray["name"]);

        if (self::arrImageValidate($imgArray) == 0) {
            if ($debug)  echo "Sorry, your file was not uploaded.";
        } else {
            if (move_uploaded_file($imgArray["tmp_name"], $target_file)) {
                if ($debug) echo "The file " . htmlspecialchars(basename($imgArray["name"])) . " has been uploaded.";
                return $uniqueFileName;
            } else {
                if ($debug) echo "Sorry, there was an error uploading your file.";
                return false;
