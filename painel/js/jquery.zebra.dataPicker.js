                    ...
                    $.each(matches, function(index, match) {
                        ...
                        switch (match.character) {
                            ...
                            case 'd':
                                ...
                                break;
                            case 'D':
                                regexp.push(plugin.settings.days_abbr ? plugin.settings.days_abbr.map(function(value) { return escape_regexp(value); }).join('|') : '[a-z\u00C0-\u024F]{3}');
                                break;
                            case 'j':
                                regexp.push('0?[1-9]|[12][0-9]|3[01]');
                                break;
                            case 'l':
                                regexp.push(plugin.settings.days ? plugin.settings.days.map(function(value) { return escape_regexp(value); }).join('|') : '[a-z\u00C0-\u024F]+');
                                break;
                            case 'N':
                                regexp.push('[1-7]');
                                break;
                            ...
                            case 'w':
                                ...
                                break;
                            case 'F':
                                regexp.push(plugin.settings.months ? plugin.settings.months.map(function(value) { return escape_regexp(value); }).join('|') : '[a-z\u00C0-\u024F]+');
                                break;
                            case 'm':
                                regexp.push('0?[1-9]|1[012]');
                                break;
                            case 'M':
                                regexp.push(plugin.settings.months_abbr ? plugin.settings.months_abbr.map(function(value) { return escape_regexp(value); }).join('|') : '[a-z\u00C0-\u024F]{3}');
                                break;
                            case 'n':
                                regexp.push('0?[1-9]|1[012]');
                                break;
            ...
            format = function(date) {
                ...
                for (i = 0; i < plugin.settings.format.length; i++) {
                    ...
                    switch (chr) {
                        ...
                        case 'M':
                            f = (Array.isArray(plugin.settings.months_abbr) && undefined !== plugin.settings.months_abbr[n - 1] ? plugin.settings.months_abbr[n - 1] : plugin.settings.months[n - 1].substr(0, 3)); // jshint ignore:line
                        ...
                        case 'D':
                            l = (Array.isArray(plugin.settings.days_abbr) && undefined !== plugin.settings.days_abbr[w] ? plugin.settings.days_abbr[w] : plugin.settings.days[w].substr(0, 3)); // jshint ignore:line
