  ...
  $.Zebra_DatePicker = function (element, options) {
    ...
    var defaults = {
      ...
      init = function (update) {
        var // the characters that may be present in the date format and that represent days, months, years, hours,
          // minutes and seconds
          date_chars = {
            days: ["d", "j", "D"],
            months: ["F", "m", "M", "n", "t"],
        ...
        if (!update) {
          ...
          original_attributes.padding_left =
            parseInt($element.css("paddingLeft"), 10) || 0;
          original_attributes.padding_right =
            parseInt($element.css("paddingRight"), 10) || 0;
        ...
        while (!format_is_valid) {
              ...
              if (plugin.settings.format.indexOf(character) > -1)
                ...
                // if time is available in the date's format and time picker is not explicitly disabled
                  ...
                  (type === "hours" ||
                    ...
                    type === "seconds" ||
                    type === "ampm") &&
                  !plugin.settings.disable_time_picker
                ) {
                  ...
                  if (type === "hours") {
                    ...
                    if ($.isArray(plugin.settings.enabled_hours))
                      plugin.settings.enabled_hour =
                        plugin.settings.enabled_hours.map(function (value) {
                    ...
                    for (
                      ...
                      if (
                        !$.isArray(plugin.settings.enabled_hours) ||
                        $.inArray(i, plugin.settings.enabled_hours) > -1
                      )
                    ...
                    if ($.isArray(plugin.settings.enabled_minutes))
                      plugin.settings.enabled_minutes =
                        plugin.settings.enabled_minutes.map(function (value) {
                          return parseInt(value, 10);
                        });
                    ...
                    for (i = 0; i < 60; i++)
                      // and add them to the lookup array if a user-defined list of values doesn't exist, or if the value is in that list
                      if (
                        !$.isArray(plugin.settings.enabled_minutes) ||
                        $.inArray(i, plugin.settings.enabled_minutes) > -1
                      )
                    ...
                    if ($.isArray(plugin.settings.enabled_seconds))
                      plugin.settings.enabled_seconds =
                        plugin.settings.enabled_seconds.map(function (value) {
                          return parseInt(value, 10);
                        });
                    ...
                    for (i = 0; i < 60; i++)
                      // and add them to the lookup array if a user-defined list of values doesn't exist, or if the value is in that list
                      if (
                        !$.isArray(plugin.settings.enabled_seconds) ||
                        $.inArray(i, plugin.settings.enabled_seconds) > -1
                      )
                    ...
                    if (
                      $.isArray(plugin.settings.enabled_ampm) &&
                      $.grep(plugin.settings.enabled_ampm, function (value) {
                        return (
                          $.inArray(value.toLowerCase(), ["am", "pm"]) > -1
                        );
                      }).length
                    )
          ...
          if (
            timepicker_config.hour_format &&
            timepicker_config.ampm &&
            timepicker_config.is12hour === false
          )
            // replace the hour's format from a 24-hour format to a 12-hour format
            plugin.settings.format = plugin.settings.format.replace(
              timepicker_config.hour_format,
              timepicker_config.hour_format.toLowerCase()
            );
        ...
        for (k in plugin.settings.custom_classes)
          if (
            plugin.settings.custom_classes.hasOwnProperty(k) &&
            custom_class_names.indexOf(k) === -1
          )
          ...
          if ($.isArray(dates) && dates.length > 0)
            ...
            $.each(dates, function () {
              ...
              for (i = 0; i < 4; i++) {
                ...
                for (j = 0; j < rules[i].length; j++)
                  rules[i][j] = isNaN(to_int(rules[i][j]))
                    ? rules[i][j]
                    : to_int(rules[i][j]);
              }
        ...
        var // cache the current date (which is either the system's date or a custom one, if given)
          date =
            plugin.settings.current_date !== false
              ? new Date(plugin.settings.current_date)
              : new Date(),
          ...
          reference_date = !plugin.settings.reference_date
            ? $element.data("zdp_reference_date") &&
              undefined !== $element.data("zdp_reference_date")
              ? $element.data("zdp_reference_date")
              : date
        ...
        // calendar is past only, ending today
          ...
          ($.isArray(plugin.settings.direction) &&
            ...
            ((tmp_start_date = check_date(plugin.settings.direction[0])) ||
              ...
              // or an integer > 0
              (is_integer(plugin.settings.direction[0]) &&
                plugin.settings.direction[0] > 0)) &&
            // and second entry is a valid date
            ((tmp_end_date = check_date(plugin.settings.direction[1])) ||
              // or a boolean FALSE
              plugin.settings.direction[1] === false ||
              // or integer >= 0
              (is_integer(plugin.settings.direction[1]) &&
                plugin.settings.direction[1] >= 0)))
        ) {
          ...
          $.isArray(plugin.settings.disabled_dates) &&
          plugin.settings.disabled_dates.length > 0
        )
          // iterate through the rules for disabling dates
          // only if there is a rule that disables *everything*
          for (var interval in disabled_dates)
            if (
              $.inArray("*", disabled_dates[interval][0]) > -1 &&
        ...
        if (
          plugin.settings.start_date &&
          typeof plugin.settings.start_date === "object" &&
          plugin.settings.start_date instanceof Date
        )
        ...
        var default_date = check_date(
          $element.val() ||
            (plugin.settings.start_date ? plugin.settings.start_date : "")
        );
        ...
        if (!update && (undefined !== start_date || undefined !== default_date))
          update_dependent(
            undefined !== default_date ? default_date : start_date
          );
        ...
        if (!(plugin.settings.always_visible instanceof jQuery)) {
          ...
          if (!update) {
            ...
            if (plugin.settings.show_icon !== false) {
              ...
              clickables = plugin.settings.open_icon_only
                ? icon
                : icon.add($element);
            ...
            clickables.on(
              "click.Zebra_DatePicker_" +
                uniqueid +
                (plugin.settings.open_on_focus
                  ? " focus.Zebra_DatePicker_" + uniqueid
                  : ""),
              function () {
                ...
                if (
                  ...
                  if (!(is_touch && !plugin.settings.readonly_element))
          ...
          if (undefined !== icon) {
            ...
            if (plugin.settings.inside)
              if (plugin.settings.icon_position === "right") {
                ...
                icon.css(
                  "right",
                  plugin.settings.icon_margin !== false
                    ? plugin.settings.icon_margin
                    : original_attributes.padding_right
                );
        ...
        if (update) {
          ...
          $(".dp_time_controls_increase .dp_time_control", datepicker).html(
            plugin.settings.navigation[2]
          );
          $(".dp_time_controls_decrease .dp_time_control", datepicker).html(
            plugin.settings.navigation[3]
          );
        ...
        var html =
          ...
          '<td class="dp_next">' +
          plugin.settings.navigation[1] +
          (is_iOS ? "&#xFE0E;" : "") +
          "</td>" +
          "</tr>" +
          "</table>" +
          '<table class="dp_daypicker' +
          (plugin.settings.show_week_number ? " dp_week_numbers" : "") +
          ' dp_body"></table>' +
        ...
        $(plugin.settings.rtl ? ".dp_next" : ".dp_previous", header).on(
          "click",
          function () {
            // if view is "months"
        ...
        if (plugin.settings.fast_navigation)
          ...
          $(".dp_caption", header).on("click", function () {
            ...
            if (view === "days")
              view =
                $.inArray("months", views) > -1
                  ? "months"
                  : $.inArray("years", views) > -1
                  ? "years"
                  : "days";
        ...
        // (directions are inverted when in RTL mode)
        $(plugin.settings.rtl ? ".dp_previous" : ".dp_next", header).on(
          "click",
          function () {
            // if view is "months"
            // increment year by 1
        ...
        cleardate.on("click", function (e) {
          ...
          else $("td.dp_selected", datepicker).removeClass("dp_selected");
          ...
          if (
            plugin.settings.onClear &&
            typeof plugin.settings.onClear === "function"
          )
        ...
        confirm_selection.on("click", function () {
          ...
          // a value is selected
          $(".dp_time_controls_increase td", timepicker)
            .first()
            .trigger("mousedown");
          clearInterval(timer_interval);
          $(".dp_time_controls_decrease td", timepicker)
            .first()
            .trigger("mousedown");
          clearInterval(timer_interval);
          ...
          if (
            ...
            typeof plugin.settings.onSelect === "function"
          ) {
            var js_date = new Date(
              selected_year,
              selected_month,
              default_day,
              timepicker_config && timepicker_config.hours
                ? selected_hour +
                  (timepicker_config.ampm &&
                  ((selected_ampm === "pm" && selected_hour < 12) ||
                    (selected_ampm === "am" && selected_hour === 12))
                    ? 12
                    : 0)
                : 0,
              timepicker_config && timepicker_config.minutes
                ? selected_minute
                : 0,
              timepicker_config && timepicker_config.seconds
                ? selected_second
                : 0
            );
            ...
            plugin.settings.onSelect.call(
              ...
              selected_year +
                "-" +
                str_pad(selected_month + 1, 2) +
                "-" +
                str_pad(default_day, 2) +
                (timepicker_config
                  ? " " +
                    str_pad(js_date.getHours(), 2) +
                    ":" +
                    str_pad(js_date.getMinutes(), 2) +
                    ":" +
                    str_pad(js_date.getSeconds(), 2)
                  : ""),
              js_date
            );
          }
        ...
        if (!(plugin.settings.always_visible instanceof jQuery)) {
          ...
          $(document).on(
            ...
            function (e) {
              ...
              if (
                ...
                ((plugin.settings.open_icon_only &&
                  plugin.icon &&
          ...
          $(document).on("keyup.Zebra_DatePicker_" + uniqueid, function (e) {
            ...
            // hide the date picker
            if (!datepicker.hasClass("dp_hidden") && e.which === 27)
              plugin.hide();
          });
        }
            ...
            if ((segments = regexp.exec(str_date))) {
              ...
              $.each(matches, function (index, match) {
                ...
                switch (match.character) {
                  ...
                  case "M":
                    ...
                    if (match.character === "D")
                      iterable =
                        plugin.settings.days_abbr || plugin.settings.days;
                    else if (match.character === "l")
                      iterable = plugin.settings.days;
                    // if month is given as month name, we'll check against the names in the used language
                    else if (match.character === "M")
                      iterable =
                        plugin.settings.months_abbr || plugin.settings.months;
                    ...
                    $.each(iterable, function (key, value) {
                      ...
                      if (
                        ...
                        switch (match.character) {
                          case "D":
                            segments[index + 1] = english_days[key].substring(
                              0,
                              3
                            );
                            break;
                          ...
                          case "F":
                            ...
                            break;
                          case "M":
                            segments[index + 1] = english_months[key].substring(
                              0,
                              3
                            );
                            original_month = key + 1;
              ...
              if (valid) {
                ...
                date = new Date(
                  ...
                  original_day || 1,
                  original_hours +
                    (original_ampm === "pm" && original_hours !== 12
                      ? 12
                      : original_ampm === "am" && original_hours === 12
                      ? -12
                      : 0),
                  original_minutes,
                  original_seconds
      ...
      format = function (date) {
        ...
        for (i = 0; i < plugin.settings.format.length; i++) {
          ...
          switch (chr) {
            ...
            case "M":
              f =
                $.isArray(plugin.settings.months_abbr) &&
                undefined !== plugin.settings.months_abbr[n - 1]
                  ? plugin.settings.months_abbr[n - 1]
                  : plugin.settings.months[n - 1].substr(0, 3); // jshint ignore:line
            ...
            case "D":
              l =
                ...
                undefined !== plugin.settings.days_abbr[w]
                  ? plugin.settings.days_abbr[w]
                  : plugin.settings.days[w].substr(0, 3); // jshint ignore:line
      ...
      generate_daypicker = function () {
        var // get the number of days in the selected month
          ...
          days_in_previous_month = new Date(
            selected_year,
            selected_month,
            0
          ).getDate(),
          ...
          days_from_previous_month =
            first_day - plugin.settings.first_day_of_week,
          i,
          html,
          day,
          real_date,
        ...
        days_from_previous_month =
          days_from_previous_month < 0
            ? 7 + days_from_previous_month
            : days_from_previous_month;
        ...
        if (plugin.settings.show_week_number)
          // column title
          html +=
            '<th scope="col">' + plugin.settings.show_week_number + "</th>";
        ...
        for (i = 0; i < 7; i++) {
          ...
          day =
            (plugin.settings.first_day_of_week +
              (plugin.settings.rtl ? 6 - i : i)) %
            7;

          html +=
            '<th scope="col">' +
            ($.isArray(plugin.settings.days_abbr) &&
            undefined !== plugin.settings.days_abbr[day]
              ? plugin.settings.days_abbr[day]
              : plugin.settings.days[day].substr(0, 2)) +
            "</th>";
        }
        ...
        for (i = 0; i < 42; i++) {
          ...
          if (
            plugin.settings.select_other_months &&
            (i < days_from_previous_month || day > days_in_month)
          ) {
            // use the Date object to normalize the date
            // for example, 2011 05 33 will be transformed to 2011 06 02
            ...
            real_date =
              real_year + str_pad(real_month + 1, 2) + str_pad(real_day, 2);
          }
          ...
          if (
            (plugin.settings.rtl && day < 1) ||
            (!plugin.settings.rtl && i < days_from_previous_month)
          )
            html +=
              '<td class="dp_not_in_month ' +
              (is_weekend ? "dp_weekend " : "") +
              (plugin.settings.select_other_months &&
              !is_disabled(real_year, real_month, real_day)
                ? "date_" + real_date
                : "dp_disabled") +
              '">' +
              (plugin.settings.select_other_months ||
              plugin.settings.show_other_months
                ? str_pad(
                    rtl_offset +
          ...
          // if this is a day from the current month
            ...
            // custom class, if any
            custom_class_name = get_custom_class(
              selected_year,
              selected_month,
              day
            );
      ...
      generate_monthpicker = function () {
        ...
        for (i = 0; i < 12; i++) {
          ...
          // else, if this the current system month, highlight it
            ...
            current_system_month === month &&
            current_system_year === selected_year
          )
            class_name += " dp_current";
          ...
          html +=
            ...
            '">' +
            ($.isArray(plugin.settings.months_abbr) &&
            undefined !== plugin.settings.months_abbr[month]
              ? plugin.settings.months_abbr[month]
              : plugin.settings.months[month].substr(0, 3)) +
            "</td>";
        }
      ...
      generate_timepicker = function () {
        ...
        html =
          ...
          (condensed ? " dp_time_controls_condensed" : "") +
          '">' +
          (plugin.settings.rtl && timepicker_config.ampm
        ...
        if (plugin.settings.rtl && timepicker_config.ampm)
          html +=
            ...
            timepicker_config.seconds
              ...
              : "") +
            '"><div>' +
            (timepicker_config.ampm_case === "A"
              ? selected_ampm.toUpperCase()
              : selected_ampm) +
            "</div></td>";
        if (timepicker_config.hours)
          html +=
            ...
            (!plugin.settings.rtl && timepicker_config.ampm)
              ...
              : "") +
            '"><div>' +
            (timepicker_config.hour_format === "h" ||
            timepicker_config.hour_format === "H"
              ? str_pad(selected_hour, 2)
              : selected_hour) +
            "</div></td>";
      ...
      generate_yearpicker = function () {
        ...
        for (i = 0; i < 12; i++) {
          ...
          if (is_disabled(selected_year - 7 + year))
            class_name += " dp_disabled";
          // else, if a date is already selected and this is that particular year, highlight it
          else if (default_year && default_year === selected_year - 7 + year)
            class_name += " dp_selected";
          // else, if this is the current system year, highlight it
          else if (current_system_year === selected_year - 7 + year)
            class_name += " dp_current";
              ...
              if ($.inArray(year, rule[2]) > -1 || $.inArray("*", rule[2]) > -1)
                if (
                  (typeof month !== "undefined" &&
                    $.inArray(month, rule[1]) > -1) ||
                  $.inArray("*", rule[1]) > -1
                )
                  // if the rules apply for the current month
                  if (
                    (typeof day !== "undefined" &&
                      $.inArray(day, rule[0]) > -1) ||
                    $.inArray("*", rule[0]) > -1
                  ) {
              ...
              (typeof end_date !== "undefined" &&
                now >
                  to_int(
                    str_concat(
                      last_selectable_year,
                      str_pad(last_selectable_month, 2),
                      str_pad(last_selectable_day, 2)
                    )
                  )))
      ...
      manage_views = function (fire_events) {
        ...
        if (daypicker.text() === "" || view === "days") {
          ...
          if (daypicker.text() === "") {
            ...
            if (
              typeof daypicker[0].getBoundingClientRect !== "undefined" &&
              typeof daypicker[0].getBoundingClientRect().height !== "undefined"
            )
        ...
        if (
          ...
          typeof plugin.settings.onChange === "function" &&
          undefined !== view
        ) {
          ...
          elements.each(function () {
            ...
            if (view === "days")
              if (
                $(this).hasClass("dp_not_in_month") &&
                !$(this).hasClass("dp_disabled")
              ) {
            ...
            // if view is "months"
              ...
              $(this).data(
                "date",
                selected_year + "-" + str_pad(to_int(matches[1]) + 1, 2)
              );
        ...
        if (view === "time" && views.length > 1) {
          ...
          if (
            ...
            (plugin.settings.always_visible &&
              plugin.settings.show_clear_date !== false)
          )
      ...
      manage_timer_controls = function (element) {
        var // are we increasing or decreasing values?
          ...
          lookup =
            timepicker_config[matches[1] + (matches[1] !== "ampm" ? "s" : "")],
          // the current value's position in the array of allowed values
          current_value_position = $.inArray(
            matches[1] !== "ampm" ? parseInt(value, 10) : value,
            lookup
          ),
          ...
          next_value_position =
            current_value_position === -1
              ? 0
              : increase
              ? current_value_position + 1 >= lookup.length
                ? 0
                : current_value_position + 1
              : current_value_position - 1 < 0
              ? lookup.length - 1
              : current_value_position - 1,
          default_date;
        ...
        value_container.text(
          str_pad(lookup[next_value_position], 2).toUpperCase()
        );
      ...
      select_date = function (year, month, day, rview, cell) {
        ...
        if (plugin.settings.always_visible || timepicker_config) {
          ...
          if (cell && view_cells) {
            ...
            if (
              rview === "days" &&
              cell.hasClass("dp_not_in_month") &&
              !cell.hasClass("dp_disabled")
            )
              plugin.show();
          }
        }
          ...
          $.each(plugin.settings.pair, function () {
            ...
            // if Zebra_DatePicker is attached to the pair element
              ...
              dp.update({
                reference_date: date,
                direction:
                  dp.settings.direction === 0 ? 1 : dp.settings.direction,
              });
    ...
    plugin.destroy = function () {
      ...
      if (
        plugin.settings.show_icon &&
        !(plugin.settings.always_visible instanceof jQuery)
      )
        $element.unwrap();
      ...
      $element.attr(
        "style",
        original_attributes.style ? original_attributes.style : ""
      );
    ...
    plugin.hide = function (outside) {
      ...
      if (
        !datepicker.hasClass("dp_hidden") &&
        (!plugin.settings.always_visible || outside)
      ) {
        ...
        if (
          ...
          typeof plugin.settings.onClose === "function"
        )
          // execute the callback function and pass as argument the element the plugin is attached to
    ...
    plugin.show = function (fire_events) {
      ...
      if (timepicker_config) {
        ...
        if (
          $.isArray(plugin.settings.enabled_hours) &&
          $.inArray(selected_hour, plugin.settings.enabled_hours) === -1
        )
          selected_hour = plugin.settings.enabled_hours[0];
        if (
          $.isArray(plugin.settings.enabled_minutes) &&
          $.inArray(selected_minute, plugin.settings.enabled_minutes) === -1
        )
          selected_minute = plugin.settings.enabled_minutes[0];
        if (
          $.isArray(plugin.settings.enabled_seconds) &&
          $.inArray(selected_second, plugin.settings.enabled_seconds) === -1
        )
          selected_second = plugin.settings.enabled_seconds[0];
        if (
          $.isArray(plugin.settings.enabled_ampm) &&
          $.inArray(selected_ampm, plugin.settings.enabled_ampm) === -1
        )
          selected_ampm = plugin.settings.enabled_ampm[0];
      }
          ...
          var // get the date picker width and height
            ...
            left =
              (undefined !== icon
                ...
                : $element.offset().left + $element.outerWidth(true)) +
              plugin.settings.offset[0],
            top =
      ...
      if (
        ...
        typeof plugin.settings.onOpen === "function"
      )
        // execute the callback function and pass as argument the element the plugin is attached to
